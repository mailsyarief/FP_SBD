...
SELECT JL.J_NAMA AS LAYANAN, COUNT(JL.J_ID) AS TOTAL
FROM NOTA N, DETIL_NOTA DN, JENIS_LAYANAN JL
WHERE DN.J_ID = JL.J_ID AND N.N_NO = DN.N_NO
GROUP BY JL.J_NAMA
ORDER BY COUNT(JL.J_ID) DESC;
...
--SIAPA MEMBER YANG CUCIANNYA DI CUCI SAMA TUKANG CUCI YANG SAMA
SELECT DISTINCT K.K_NAMA, Z.K_KODE, M.M_NAMA
FROM NOTA N, MEMBER_TABLE M, DETIL_KARYAWAN DK, KARYAWAN K,
(SELECT DISTINCT K.K_KODE FROM KARYAWAN K WHERE K.K_KODE = 'K2101') Z
WHERE M.M_NO = N.M_NO AND N.N_NO = DK.N_NO AND DK.K_KODE = Z.K_KODE

--GOLONGKAN KARYAWAN CABANG--
SELECT K.K_NAMA, K.K_KODE AS OUTLETSATU
FROM KARYAWAN K, OUTLET O
WHERE K.K_KODE LIKE 'K1%'

--CARI PEGAWAI PALING SERING KERJA

SELECT K.K_NAMA AS NAMA, COUNT(DK.K_KODE) AS TOTAL
FROM KARYAWAN K, DETIL_KARYAWAN DK, NOTA N, DETIL_NOTA DN
WHERE DN.N_NO = DK.N_NO AND DK.K_KODE = K.K_KODE AND N.N_NO = DN.N_NO
GROUP BY K.K_NAMA
ORDER BY TOTAL DESC


--TRANSAKSI DIATAS 100RB V2--
SELECT DISTINCT M.M_NO AS ID, M.M_NAMA AS NAMA, N.N_NO AS NOTA, Z.JUMLAH AS TOTAL
FROM MEMBER_TABLE M, NOTA N, JENIS_LAYANAN JL,
(SELECT N.N_NO, SUM((JL.J_HARGA * DN.DN_KUANTITAS)-(JL.J_HARGA * DN.DN_KUANTITAS * (N.N_DISKON/100)) - N.N_UANGMUKA) AS JUMLAH
FROM MEMBER_TABLE M, NOTA N, DETIL_NOTA DN, JENIS_LAYANAN JL
WHERE M.M_NO = N.M_NO AND N.N_NO = DN.N_NO AND JL.J_ID = DN.J_ID
GROUP BY N.N_NO) Z
WHERE M.M_NO = N.M_NO AND N.N_NO = Z.N_NO AND Z.JUMLAH > 100000
ORDER BY TOTAL DESC

--SIAPA AJA DAN BERAPA KALI  CUCI PADA BULAN X--
SELECT Z.M_NAMA, COUNT(Z.M_NAMA) FROM
(SELECT DISTINCT N.N_NO, M.M_NAMA , N.N_TGLMASUK
FROM MEMBER_TABLE M, NOTA N, DETIL_NOTA DN, JENIS_LAYANAN JL
WHERE N.N_TGLMASUK LIKE '%__-SEP__%' AND N.M_NO = M.M_NO) Z
GROUP BY Z.M_NAMA

>>>>>>> 58599f0fb7a30f6dcdefbc8a10380addbc98e72d
